...
def sliding_window(image, window_size, step_size):
    ...
    for y in range(75, image.shape[0], step_size[1]):
...
cap=cv2.VideoCapture('Clueless_Pedestrians_compress.mp4')
prev_frame_time = 0
new_frame_time = 0
...
while True:
    ...
    cv2.putText(clone, fps, (7, 70), font, 3, (100, 255, 0), 3, cv2.LINE_AA)


    cv2.imshow('webcam before nms', image)
    cv2.imshow('webcam after nms', clone)

    if cv2.waitKey(1) == 27:  # Exc key
        break
